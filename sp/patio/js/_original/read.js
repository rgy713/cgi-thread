        ...
        $("div.main").children("div[id^=d]").each(function () {
            ...
            if (refReses.length < NUMBER_OF_ANCHOR_MADE_REF_COUNT_EXEMPT) {
                refReses.each(function () {
                    var element = $(this);
                    var refResNo = parseInt(element.attr("name").replace(/^d/, ""), 10);
                    if (refResNo >= resNo) {
...
$(function() {
    ...
    // モーダルを表示する
    $(document).on('click', 'div.comment a.scroll', function(e) {
	...
	e.preventDefault();
	var that = $(this);
	var current = $('#'+that.attr('name')).html();
	var showPopup = function(html){
			    $('#modale').show();
                	    $('<div id="modal' + i + '" class="modal"><img src="./img/close.png" alt="close" width=20" height="20" class="close">'+html+'</div>').appendTo('#modal');
	                    $('div#modal' + i).css({width: 'calc(100% - 30px)', 'left': '5px', 'top' : that.offset().top+ that.height() + 10  +'px'});
   			    if(document.getElementById(that.attr('name'))==null){
				$('#modal'+i+'>dl>dt>.hatugen_blue').remove();
				$('#modal'+i+'>dl>dt>.hatugen_red').remove();
			    }else if ($("#modal"+i+" .hatugen_blue").size()+$("#modal"+i+" .hatugen_red").size()===0){
				$('#modal'+i+' .res_poster_id').append($('#'+that.attr('name')+'>dl>dt>.hatugen_blue').clone());
				$('#modal'+i+' .res_poster_id').append($('#'+that.attr('name')+'>dl>dt>.hatugen_red').clone());
		    	    }
	                    $('div#modal' + i).show();
	};
	if(current){
		showPopup(current.replace(/class="refRes"/,'style="display:none"').replace(/<span class="hi_sanshou">[^<]+<\/span>/,''));
	}else{
	        $.ajax({
        	    type: "GET",
	            url: './read.cgi?l=' + $(this).attr('name').replace('d', '') + '&no=' + no,
        	    context: this,
	            success: function(data){
        	        if(data.match('<h3>記事が見当たりません</h3>') === null && data.match('<h3>ERROR !</h3>') === null) {
                	    var html = $(data).find('#'+that.attr('name')).html();
			    if(!html)
			    showPopup("<h2>このスレは存在しません</h2>");
			    else
			    showPopup(html.replace(/<span class="hatugen_(blue|red)">[^<]+<\/span>/,''));
        	        }else
			    showPopup("<h2>このスレは存在しません</h2>");
            	    }
	        });
    ...
    // モーダルを隠す
    $(document).on('click', 'div.modal img.close', function() {
        var id = parseInt($(this).closest('div.modal').attr('id').replace('modal', ''));
        $(this).closest('div#modal').find('div.modal').each(function(i, data) {
            if(parseInt($(data).attr('id').replace('modal', '')) >= id) {
                $(data).remove();
                $(data).hide();
            }
        });
