...
;######################################################################
;# jcodeg.pl
;#          Modified by 町の妖精さん <strangeworld@jo.st22.arena.ne.jp>
;#                             http://jo.st22.arena.ne.jp/strangeworld/
;#              http://jo.st22.arena.ne.jp/cgi-bin/strangeworld/bbs.cgi
;#
;# 歌代さんの jcode.pl に、gama さんの getcode.pl から getcode() を
;# 組み込んで、文字種判定を強化したものです。
;#
;# この改変について、編集著作権は主張しません。
;#
;# 2000/02/29
;######################################################################
;# 2004/06/05 jcode.pl v2.13.xの魂を注入。 by がっぱい
;#  ＊v2.13.1 v2.13.2 v2.13.3の存在を誰も知らないので
;#    むしゃくしゃして作った。
;#    今は反省している。
;######################################################################
...
;#
;; $rcsid = q$Id: jcode.pl,v 2.13.x 2002/04/07 08:13:57 utashiro Exp $;
;######################################################################
;#  getcode.pl
;#
;#  半角カナのコード系判別をめざして
...
sub init {
    ...
    local($tokuho_check)	= $_[0];
    if($tokuho_check eq "tokuho_check"){
        return(9999999999);# （藁
    }
...
sub tr {
    ...
    if (!defined($prev_from) || $from ne $prev_from || $to ne $prev_to || $opt ne $prev_opt) {
