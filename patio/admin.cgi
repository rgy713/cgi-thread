...
if($in{'cookie_id'} eq 'on'){
	...
	if (!defined($cookie->('HISTORY_ID'))) {
		...
		if(!( grep $_ eq $history_id , @thread_Edit)){
			&error("認証エラー");
		}
	}
} else{
...
sub manage_log {
		...
		if($in{'submitted'} eq 1 and time - $in{'time'} < 3) {
			$in{'body'} =~ s/&lt;/</g;
			$in{'body'} =~ s/&gt;/>/g;
			$in{'body'} =~ s/>>([0-9]+)/&gt;&gt;$1/g;
...
スレッド名：@title[1]<br>
スレッド番号：$in{'file'} レス数：$i
</h3>
EOM

		if($in{'submitted'} eq 1) {
			print <<"EOM";
<h4 class=\"hide-on-reload\">保存しました。</h4>
EOM
}

print <<"EOM";
<form action=\"$admincgi\" name=\"logform\" id=\"logform\" method=\"post\">
<input type=\"hidden\" name=\"pass\" value=\"$in{'pass'}\">
<input type=\"hidden\" name=\"mode\" value=\"admin\">
<input type=\"hidden\" name=\"file\" value=\"$in{'file'}\">
...
<input type=\"button\" id=\"clicker\" value=\"ログを保存\">
</form>\n<script>
jQuery(function(){
if(sessionStorage.getItem(\"logsave\")){
jQuery(\".hide-on-reload\").remove();
}else{
sessionStorage.setItem(\"logsave\",\"1\");
}

jQuery('\#clicker\').on('click',function(event){
event.preventDefault();
sessionStorage.removeItem('logsave');
jQuery('\#time').val(Math.floor(+new Date()/1000));
