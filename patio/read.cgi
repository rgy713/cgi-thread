...
#	elsif ($key ne '1') {$job = "";}
	...
	$hist_pre = '__hist';
	$cookiea_pre='__cookiea';
	# ID 毎投稿数カウント機能 親レス投稿者IDをカウント
	$resCountPerPoster{$idcrypt} = [$no2];
	# 書込ID 毎投稿数カウント機能 親レス投稿者 書込IDをカウント
	if($log_history_id ne ''){
		$resCountPerPoster{($log_history_id.$hist_pre)} = [$no2];
	}
	# Cookie_A 毎投稿数カウント機能 親レス投稿者 Cookie_A をカウント
	if($log_cookie_a ne ''){
		$resCountPerPoster{($log_cookie_a.$cookiea_pre)} = [$no2];
	}
...
EOM
	...
	foreach my $res_log_array_ref (@log[1..$#log]) {
		# ID, 書込ID, CookieA 毎投稿数カウント機能 表示範囲内 子レスの同一IDによる投稿をカウント
		my ($res_no, $id, $hist_id, $cookiea) = @{$res_log_array_ref}[0, 15, 19, 18];
		if ( $start_res_no == 0) {
			$start_res_no = $res_no;
		}
		$last_res_no = $res_no;
		...
		if ($hist_id ne ''){
			if (exists($resCountPerPoster{($hist_id.$hist_pre)})) {
				push(@{$resCountPerPoster{($hist_id.$hist_pre)}}, $res_no);
			} else {
				$resCountPerPoster{($hist_id.$hist_pre)} = [$res_no];
			}
		}
		if($cookiea ne ''){
			if (exists($resCountPerPoster{($cookiea.$cookiea_pre)})) {
				push(@{$resCountPerPoster{($cookiea.$cookiea_pre)}}, $res_no);
			} else {
				$resCountPerPoster{($cookiea.$cookiea_pre)} = [$res_no];
			}
		}
		# 表示内容分岐機能 子レス判定
...
<h1 class="thread">$sub &nbsp;&nbsp; フォルダ_ <a href="#last_res">last</a>: $thread_savefolder_html</h1>
EOM
...
# IDを表示
	if($idkey && $idcrypt) {
		print " ID:<span class=\"res_poster_id\">$idcrypt</span></span>";
		# ID毎投稿数カウント機能 投稿数を出力
		print first_make_number_utterances($idcrypt);
	} else {
		print "</span>";
	}
	...
	if ($log_history_id ne '') {
		...
		# 書込ID毎投稿数カウント機能 投稿数を出力
		print first_make_number_utterances(($log_history_id.$hist_pre));
	}
	...
	if ($log_cookie_a ne '') {
		...
		# CookieA毎投稿数カウント機能 投稿数を出力
		print first_make_number_utterances(($log_cookie_a.$cookiea_pre));
	}
...
#			if (defined($imgex{$ex})) {
				...
				if($thumb_path && $width > 0 && $height > 0) {
					...
					print "<a class=\"pslink\" data-w=\"$w\" data-h=\"$h\" href=\"$uplurl/$img_folder_number/$tim-$i$ex\" target=\"_blank\"><img src=\"$thumb_path\" align=\"top\" border=\"0\" width=\"$width\" height=\"$height\" hspace=\"3\" vspace=\"5\"></a>\n";
					# オリジナル画像ファイル名取得
					$prt = $ENV{'REQUEST_URI'};
					$prt =~ s/\/[^\/]+$//;
					# TODO 20171027 サムネイル画像と、元画像の幅、高さのサイズを出力します。					
					push(@img_filename, "■ <a class=\"pslink\" data-w=\"$w\" data-h=\"$h\" href=\"$uplurl/$img_folder_number/$tim-$i$ex\">http://$ENV{'HTTP_HOST'}$prt/$img_filename_prefix$img_folder_number/$tim-$i$ex</a> ■  サムネイル→縦：$thumb_h px 横：$thumb_w px 画像→縦：$h px　横：$w px");
				}
			} else {
				print "[<a href=\"$uplurl/$img_folder_number/$tim-$i$ex\" target=\"_blank\">$tim-$i$ex</a>]\n";
			}
...
# IDを表示
	if($idkey && $idcrypt) {
		print " ID:<span class=\"res_poster_id\">$idcrypt</span></span>";
		# ID毎投稿数カウント機能 投稿数を出力
		print make_number_utterances($idcrypt);
	} else {
		print "</span>"
	}
	...
	if ($log_history_id ne '') {
		...
		# 書込ID毎投稿数カウント機能 投稿数を出力
		print make_number_utterances(($log_history_id.$hist_pre));
	}
	...
	if ($log_cookie_a ne '') {
		...
		# CookieA毎投稿数カウント機能 投稿数を出力
		print make_number_utterances(($log_cookie_a.$cookiea_pre));
	}
...
#			if (defined($imgex{$ex})) {
				...
				if($thumb_path && $width > 0 && $height > 0) {
					# サムネイル画像およびオリジナル画像へのリンク HTML出力
					print "<a class=\"pslink\" href=\"$uplurl/$img_folder_number/$tim-$i$ex\"  data-w=\"$w\" data-h=\"$h\" target=\"_blank\"><img src=\"$thumb_path\" align=\"top\" border=\"0\" width=\"$width\" height=\"$height\" hspace=\"3\" vspace=\"5\"></a>\n";
					# オリジナル画像ファイル名取得
					$prt = $ENV{'REQUEST_URI'};
					$prt =~ s/\/[^\/]+$//;
					push(@img_filename, "■ <a data-w=\"$w\" data-h=\"$h\" href=\"$uplurl/$img_folder_number/$tim-$i$ex\">http://$ENV{'HTTP_HOST'}$prt/$img_filename_prefix$img_folder_number/$tim-$i$ex</a> ■ サムネイル→縦：$height px 横：$width px 画像→縦：$h px　横：$w px");
				}
			} else {
				print "[<a href=\"$uplurl/$img_folder_number/$tim-$i$ex\" target=\"_blank\">$tim-$i$ex</a>]\n";
			}
...
EOC
		...
		my $res_no_present;
		if ($all_flg){
			$res_no_present = "<h2>[全レス表\示中]</h2>\n";
			my $no_count = int($last_res_no/$middle_number);
			for(my $num_idx = 0;$num_idx<$no_count;$num_idx++){
				$res_no_present .= "<h2>". ($num_idx+1)*$middle_number."</h2>\n"
			}
		}else{
			my $start_num = (int($start_res_no/10) +1)*10;
			my $no_count = int(($last_res_no-$start_num)/$middle_number);
			for(my $num_idx = 0;$num_idx<$no_count;$num_idx++){
				$res_no_present .= "<h2>".($start_num + ($num_idx+1)*$middle_number)."</h2>\n"
			}
		}
		if ($res_no_present ne ''){
			$is_branching_contents=1;
			$common_display_contents_at_bottom_of_page =$res_no_present.$common_display_contents_at_bottom_of_page;
		}
...
<script>
\$(function(){
\$('.comment-img').each(function(i,elm){
 var lpd = 0;
 \$(elm).find('a').each(function(j,tgt){
   var pid;
   if(\$(tgt).hasClass('pslink')){
     pid = j;
     lpd = j;
   }else{
     pid = j - lpd - 1;
   ...
   \$(tgt).click(function(e){
     e.preventDefault();
     openPS(\$(e.target).closest('a'), pid);
   });
 });
});

\$('.pswp__button').click(function(e){
  e.preventDefault();
});

var openPS = function(clk, tid){
var pswpElement = document.querySelectorAll('.pswp')[0];

// build items array
var items = [];

\$(clk).closest('.comment-img').find('.pslink').each(function(j,tgt){
 items.push({
   src: tgt.href,
   h: \$(tgt).data('h'),
   w: \$(tgt).data('w')
...
EOM
		...
		# <input type="text" name="sub" size="$sublength" value="$resub" maxlength="$sublength">

		if ($job eq "preview") {
			print <<EOM;
  <th bgcolor="$col5" width="90"><font color="$col4">題名</font></th>
  <td bgcolor="$col5" >
<input type="text" name="sub" size="$sublength" value="" maxlength="$sublength"> <font color="$col4">内容を表\す題名をつけてください</font>
EOM
			$submit = 'スレッドを生成';
		} elsif ($job eq "resview") {
			print <<EOM;
  <th bgcolor="$col5" width="90"><font color="$col4">題名</font></th>
  <td bgcolor="$col5" >
<input type="text" name="sub" size="$sublength" value="t" maxlength="$sublength"> <font color="$col4">内容を表\す題名をつけてください</font>
EOM
			$submit = '返信する';
			...
			if ($restitle) {
				print <<EOM;
  <th bgcolor="$col2" width="90">題名</th>
  <td>
<input type=\"input\" name=\"sub\" size=\"$sublength\" value=\"\" maxlength=\"$sublength\">
...
EOM
			if ($restitle) {
				print "<input type=\"input\" name=\"sub\" size=\"$sublength\" value=\"t\" maxlength=\"$sublength\">\n";
				$submit = ' 返信する ';
                if ($show_sage_checkbox) {
                    print "<input type=\"checkbox\" name=\"sage\" value=\"1\"";
                    if ($cthread_sage eq '1') { print " checked"; }
...
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">名前</th>
  <td>
EOM
		# 名前欄非表示機能 (レスフォーム表示時のみ)
		if (($job ne '' && $job ne 'resview') || !$mu->is_hide_name_field_in_form($thread_title, \@hide_form_name_field)) {
			print "    <input type=\"text\" name=\"name\" size=\"30\" value=\"$cnam\" maxlength=\"20\">　「名前#任意の文字列」でトリップ生成\n";
		}
		print <<"EOM";
  </td>
</tr>
...
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">ID</th>
  <td>
		<input type="text" name="ID" size="30" value="" maxlength="20">
  </td>
</tr>
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">書込ID</th>
  <td>
		<input type="text" name="historyID" size="30" value="" maxlength="20">
  </td>
</tr>
EOM
		...
		if ($in_mail == 2) {
			...
			print <<"EOM";
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">ホスト</th>
  <td bgcolor=\"$col2\">
	<input type=\"text\" name=\"email\" size=\"30\" value=\"$ceml\">
EOM
...
EOM
			...
			print <<EOM;
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">URL</th>
  <td bgcolor="$col2"><input type="text" name="url" size="45" value="$curl">
EOM
...
EOM
		...
		$maxdata = int(	$maxdata /1000000*10)/10;
		$maxdata_gif = int($maxdata_gif /1000000*10)/10;

		# 画像フォーム
		if ($image_upl && ($job eq "preview" || $job eq "resview" || $restitle && ($job eq 'new' || $job eq ""))) {
			print "<tr bgcolor=\"$col2\">\n";
			print "<td bgcolor=\"$col2\" width=\"90\" align=\"center\">";
			...
			if ($show_image_upl_buttons) {
				if ($upl_increase_num > 0) {
					...
					print "> 4枚以上<br>\n";
			print "JPG/PNG ";
			print " $maxdata MB、GIF $maxdata_gif MBまで）<br>";
				}
				foreach my $i (1 .. 3) {
			...
			print join('/', map { $_ =~ s/^\.//; uc($_); } sort(grep { $imgex{$_}; } keys(%imgex)));
			print "</span></td>";
			print "<td bgcolor=\"$col2\">画像の添付は次のプレビュー画面で行えます\n";
			print "</td></tr>\n";
		...
		print <<EOM;
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">パスワード</th>
  <td bgcolor="$col2">
...
EOM
		if(($webprotect_auth_new && ($job eq "new" || $job eq "preview")) || $webprotect_auth_res && ($job eq "" || $job eq "resview")) {
			print <<EOM;
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">登録ID</th>
  <td bgcolor="$col2">
  <input type="text" name="user_id" size="30" value="$cuser_id">
  </td>
</tr>
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">登録パスワード</th>
  <td bgcolor="$col2">
...
EOM
		...
		if ($is_recaptcha_enabled) {
			print <<"EOM"
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">投稿キー</th>
  <td bgcolor="$col2">
...
EOM
			...
			# 入力フォーム
			print qq |<tr bgcolor="$col2"><th bgcolor="$col2" width="90">投稿キー</th>|;
			...
			print qq |<tr bgcolor="$col2"><th bgcolor="$col2" width="90">投稿キー</th>|;
			print qq |<td bgcolor="$col2"><input type="text" name="regikey" size="6" value="$in{'regikey'}" style="ime-mode:inactive">\n|;
			print qq |（投稿時 <img src="$registkeycgi?$in{'str_crypt'}" align="absmiddle" alt="投稿キー"> を入力してください）</td></tr>\n|;
			print qq |<input type="hidden" name="str_crypt" value="$in{'str_crypt'}">\n|;
		}

		print <<EOM;
<tr bgcolor="$col2">
  <th bgcolor="$col2" width="90">コメント</th>
  <td bgcolor="$col2">
EOM
...
EOM
		...
		if ($editpreview) {
			print <<"EOM";
<textarea name="comment" cols="$cols" rows="$rows" wrap="soft" name="fokasuidou" id="fokasuidou" readonly style="color:#999999">
EOM
		} else {
			print <<"EOM";
<textarea name="comment" cols="$cols" rows="$rows" wrap="soft" name="fokasuidou" id="fokasuidou">
EOM
		}
...
EOM
		...
		if ($job eq "new") {
			print '<table width="95%"><tr><td align="left" nowrapper-main>';
		}
...
$msg =~ s/&gt;&gt;([0-9]?[0-9]?[0-9]?)/<a href=\"#$1\" class=\"scroll\" name="\d$1\">&gt;&gt;$1<\/a>/gi;
...
# 発言数の表示
sub make_number_utterances{
	my ($make_id) = @_;
	my $resCount = @{$resCountPerPoster{$make_id}};
	if ($resCount > 4) {
		# Binary Search
		my ($low, $high) = (0, $resCount);
		while($low < $high) {
			my $cursor = int(($high-$low)/2 + $low);
			if($resCountPerPoster{$make_id}->[$cursor] < $no) {
				$low = $cursor + 1;
			} else {
				$high = $cursor;
			}
		}
		my $i = $resCountPerPoster{$make_id}->[$low] == $no ? ($low+1) : -1;
		return " <span class=\"hatugen_red\">[$i/$resCount]</span>";
	} elsif ($resCount > 1) {
		# Binary Search
		my ($low, $high) = (0, $resCount);
		while($low < $high) {
		...
		my $i = $resCountPerPoster{$make_id}->[$low] == $no ? ($low+1) : -1;
		return " <span class=\"hatugen_blue\">[$i/$resCount]</span>";
	}
	return '';
}
# 発言数の表示
sub first_make_number_utterances{
	my ($make_id) = @_;
	my $resCount = @{$resCountPerPoster{$make_id}};
	if ($resCount > 4) {
		# Binary Search
		...
		return " <span class=\"hatugen_red\">[1/$resCount]</span>";
	} elsif ($resCount > 1) {
		# Binary Search
		my ($low, $high) = (0, $resCount);
		while($low < $high) {
		...
		return " <span class=\"hatugen_blue\">[1/$resCount]</span>";
	}
}
