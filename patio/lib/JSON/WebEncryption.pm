...
sub encode
{
    ...
    my @segment;
    push @segment, _encode_base64url( encode_json($header) );
    push @segment, _encode_base64url( $encrypted_key );
    push @segment, _encode_base64url( $iv );
    push @segment, _encode_base64url( $ciphertext );

    my $to_be_signed = join('.', @segment);

    my $icheck = _encode_base64url( &$integrity_fn( $to_be_signed ) );
...
sub decode
{
    ...
    #
    my $header = decode_json( _decode_base64url( $segment[0] ) );
    ...
    my $encrypted_key = _decode_base64url( $segment[1] );
    my $iv            = _decode_base64url( $segment[2] );
    my $ciphertext    = _decode_base64url( $segment[3] );
    ...
    if( $icheckB64 ne _encode_base64url( &$integrity_fn($signed_section) ) )
    {
