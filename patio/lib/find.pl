...
sub find {
	local($target,$alarm,$next,$back,$enwd,$imgfind,@log1,@log2,@log3,@wd,@imgfile_expr,@imgfile_expr_list,@findtypes,$find_log_append_contents);

	# 検索種別定義
	@findtypes = (["キーワード検索", "keyword"], ["登録ID検索", "user_id"], ["ユーザID検索", "patio_id"]);

    # 検索を実行するかどうか/Whether to perform a search
    my $performFinding = $in{'word'} || $in{'minus1'} || $in{'minus2'} || $in{'lower'} || $in{'upper'} || $in{'dateFrom'} || $in{'dateTo'}&& ($in{'type'} eq "keyword" && $in{'s'} || $in{'n'} || $in{'r'} || $in{'c'}) || $in{'type'} eq "user_id" || $in{'type'} eq "patio_id";
...
EOM
    ...
    foreach ("AND", "OR") {
		print "<label><input type=\"radio\" name=\"op\" value=\"$_\"";
		if ($in{'op'} eq $_ or $in{'op'} eq '' and $_ eq 'AND') {
			print "checked";
		}
		print ">$_</label>\n";
   }
    ...
    foreach (1, 2, 3, 5, 7, 9, 10, 100, 150, 200, 250) {
        if ($in{'vw'} == $_) {
            print "<option value=\"$_\" selected>$_ 件\n";
        }
        else {
            print "<option value=\"$_\">$_ 件\n";
        }
    }
    ...
    print "</select><br>";
    if (!$in{'minus1'}) {
        print "<br><label>マイナス検索1&nbsp&nbsp&nbsp<input type = \'text\' name = \'minus1\' size=\'38\' value = \'$keyword[0]\'/><p></p>";
    }
    elsif ($in{'minus1'}) {
        print "<br><label>マイナス検索1&nbsp&nbsp&nbsp<input type = \'text\' name = \'minus1\' size=\'38\' value = \'$in{'minus1'}\'/><p></p>";
    }
    #   print "<br><label>マイナス検索1&nbsp&nbsp&nbsp<input type = \'text\' name = \'minus1\' size=\'38\' value = \'$in{'minus1'}\'/><p></p>";
    print "<br><label>マイナス検索2&nbsp&nbsp&nbsp<input type = \'text\' name = \'minus2\' size=\'38\' value = \'$in{'minus2'}\'/><p></p>";
    print "<br>レス数の下限 ";
    print "<input type=text name=\"lower\" size=\'8\' value=\"$in{'lower'}\"> ～上限 <input type=text name=\"upper\" size=\'8\' value=\"$in{'upper'}\"><p></p>";
    print "<br>期間指定(最終書込時間) ";


    # setting previous day if $d is setted in init.cgi point 25
    if ($in{dateFrom} == '' && $d != 0) {
        $getday = scalar localtime ($time - (60 * 60 * 24 * $d));
        print "<input type=date name=\"dateFrom\" value=\"" . $getday->date . "\"> ";
    }
    if($in{dateFrom} || $d == 0){
        print "<input type=date name=\"dateFrom\" value=\"$in{'dateFrom'}\"> ";
    }

    print "～ <input type=date name=\"dateTo\" value=\"$in{'dateTo'}\">";
...
EOM
	...
	if ($performFinding) {
        ...
        # 検索キーワードを半角スペースで分割
        #括弧内の部分文字列を取得する
        my $search_words = $enc_cp932->decode($in{'word'});
        my @my_array = $search_words =~ /\[(.*?)\]/g;
        my @list_s_words=();
        if (@my_array != ''){
            my $len_ma = @my_array;
            my @sec_words = split_bar(@{my_array[0]});
            push(@list_s_words,@sec_words);
            for(my $n=1;$n<$len_ma;$n++){
                @sec_words = split_bar(@{my_array[$n]});
                @list_s_words = make_array_list( \@list_s_words ,\@sec_words );
            }
            sub split_bar{
                local ($words) = @_;
                return split(/\|/, $words);
            ...
            sub split_space{
                local ($words) = @_;
                return split(/\s+/, $words);
            }
            sub make_array_list{
                my ($f_w_ref,$s_w_ref)=@_;
                my @f_w = @{$f_w_ref};
                my @s_w = @{$s_w_ref};
                my @list = ();
                my $list_idx=0;
            ...
            my @out_braket_words=();
            my $first_text = (split(/\[/, $search_words, 2))[0];
            if ($first_text){
                push(@out_braket_words,split_space($first_text));
            }
            my @middle_text = $search_words =~ /\](.*?)\[/g;
            if (@middle_text){
                foreach my $m_idx(@middle_text){
                    push(@out_braket_words,split_space( $m_idx));
                }
            }
            my @last_text_list = split(/\]/, $search_words);
            my $last_bracket= @last_text_list;
            my $last_text='';
            if ($last_bracket>1){
                $last_text = @last_text_list[$last_bracket-1];
                if($last_text =~ /\[/){
                    $last_text = '';
                }
            }
            if ($last_text){
                push(@out_braket_words,split_space( $last_text));
            }

            foreach $each(@list_s_words){
                my $e_size = @$each;
                if($e_size){
                    push(@{$each},@out_braket_words);
                }else{
                    push(@{$each}, $each);
                    push(@{$each},@out_braket_words);
                }
            ...
            push(@{$list_s_words[0]}, split_space($search_words));
        }
        # 画像ファイル名検索正規表現配列に追加
        if ($in{'c'}){
            my $img_filename_prefix_reg =  make_half_full_regex($img_filename_prefix);
            my $imgfile_expr_idx = 0;
            foreach $each(@list_s_words){
                @imgfile_expr = ();
                foreach $in_each(@{$each}){
                    if ($in_each =~ /^$img_filename_prefix_reg/) {
                        my $word = $in_each;
                        $imgfind = 1;
                        $word =~ s/^$img_filename_prefix_reg//;
                ...
                push(@{$imgfile_expr_list[$imgfile_expr_idx]},@imgfile_expr);
                $imgfile_expr_idx++;
            }
        }
            ...
            while (<$log_fh>) {
				...
				if($in{'type'} eq "keyword") {
                    ...
                    if ($in{'c'} || $in{'r'}) {
                        ...
                        while (<$thread_log_fh>) {
                            ...
                            if ($imgfind) {
                                ...
                                for my $each (@imgfile_expr_list){
                                    for my $expr (@{$each}) {
                                        my $f = 0;
                                        for my $filename (@filenames) {
                                            my $expr_reg = make_half_full_regex($expr);
                                            if ($filename =~ /$expr_reg/) {
                                                $f = 1;
                                                last;
                                            }
                                        }
                    ...
                    foreach $each(@list_s_words){
                        ...
                        foreach my $wd (@{$each}) {
                            ...
                            #正規表現関数を呼び出す
                            my $key_regular = make_half_full_regex($wd);

                            if ($target =~ /$key_regular/) {
                                $is_finded = 1;
                                if ($in{'op'} eq 'OR') {
                                    last;
                                }
				...
				if($in{'type'} eq "user_id" || $in{'type'} eq "patio_id") {
					...
					THREADLOG_LOOP: while (<$thread_log_fh>){
                        ...
                        $target = $in{'type'} eq "user_id" ? (split(/<>/))[16] : (split(/<>/))[15];
                        if ($target eq ''){
                            next;
                        }
						$target = $enc_cp932->decode($target);
						# 検索対象をすべて小文字に
                        ...
                        foreach $each(@list_s_words){
                            foreach my $wd (@{$each}) {
                                ...
                                if ($target =~ /$key_regular/) {
                                    $idfound_flgs{$wd} = 1;
                                    if ($in{'op'} eq 'OR' || keys %idfound_flgs == scalar(@{$each})) {
                                        $flg = 1;
                                        last THREADLOG_LOOP;
                                    }
                                }
...
#                $in{'lower'} =~ s/\D//g;
#                $in{'upper'} =~ s/\D//g;
                if($in{'lower'} !~ /[\N{U+0030}-\N{U+0039}]/) {

                    $in{'lower'} = to_halfwidth($enc_cp932->decode($in{'lower'}));
                    #$in{'lower'} = to_halfwidth($in{'lower'});
                }
                if($in{'upper'} !~ /[\N{U+0030}-\N{U+0039}]/) {

                    $in{'upper'} = to_halfwidth($enc_cp932->decode($in{'upper'}));
                }
                # 対象であれば、検索結果として出力
                #下限と上限をチェックする
                if ($flg && $in{'lower'} && $in{'upper'} == '') {
                    if ($res >= $in{'lower'}) {
                        #マイナス検索の有無を確認する
                        if (check_minus($sub) == 0) {
                ...
                elsif ($flg && $in{'lower'} && $in{'upper'}) {
                    if ($res >= $in{'lower'} && $res <= $in{'upper'}) {
                        #マイナス検索の有無を確認する
                        if (check_minus($sub) == 0) {
                            push(@matched_thread_info_log_array, \@splitted_thread_info_log_line);
                        }
...
EOM
		...
		for (my $i = $p; $i < ($p + $in{vw}) && $i < scalar(@matched_thread_info_log_array); $i++) {
			...
			my $enc_sub = $enc_cp932->decode($sub);
			...
			foreach $each(@list_s_words) {
				foreach my $wd (@{$each}) {
					...
					if ($in{'cs'} eq 'ON') {
						$enc_sub =~ s/($key_regular)/<m>$1<\/m>/ig;
					}else{
						$enc_sub =~ s/($key_regular)/<m>$1<\/m>/g;
					}
				}
			}
			$enc_sub = $enc_cp932->encode($enc_sub);

			print "<td bgcolor=\"$col2\" class=\"test5\">";
			print "<a href=\"$readcgi?no=$no\">$enc_sub</a></td>";
...
# マイナス検索ボックスに入力された値の中の要素を削除する
sub check_minus {
    my ($s) = @_;
    $s = $enc_cp932->decode($s);
    my $status = 0;
    if ($in{'cs'} eq 'ON') {$s = lc($s);}
    my $search_minuse1 = $enc_cp932->decode($in{'minus1'});
    foreach my $minus1 (split(/\s/, $search_minuse1)) {
        if ($in{'cs'} eq 'ON') {
            $minus1 = lc($minus1);
        }
        #calling making regex function
        my $minus1_key_regular1 = make_half_full_regex($minus1);
        if ($s =~ /$minus1_key_regular1/) {
            $status = 1;
            last;
        }
    }
    my $search_minuse2 = $enc_cp932->decode($in{'minus2'});
    foreach my $minus2 (split(/\s/, $search_minuse2)) {
        if ($in{'cs'} eq 'ON') {
            $minus2 = lc($minus2);
        }
        #calling making regex function
        my $minus1_key_regular2 = make_half_full_regex($minus2);
        if ($s =~ /$minus1_key_regular2/) {
            $status = 1;
            last;
        }
    }
...
#半角と全角を変換する正規表現を作成する
sub make_half_full_regex {
    my ($wd) = @_;
    my $key_regex = '';
    for ($i = 0; $i < length($wd); $i++) {
        my $nth = substr($wd, $i, 1);
        if ($nth =~ /[\N{U+0030}-\N{U+0039}\N{U+FF10}-\N{U+FF19}\N{U+0041}-\N{U+005A}\N{U+0061}-\N{U+007A}\N{U+FF21}-\N{U+FF3A}\N{U+FF41}-\N{U+FF5A}]/) {
            #if half-width number
            if ($nth =~ /[\N{U+0030}-\N{U+0039}]/) {
                $key_regex = $key_regex . '[' . $nth . '|' . to_fullwidth($nth) . ']';
            }
            #if full-width number
            elsif ($nth =~ /[\N{U+FF10}-\N{U+FF19}]/) {
                $key_regex = $key_regex . '[' . $nth . '|' . to_halfwidth($nth) . ']';
            }
            #if half-width english letter
            elsif ($nth =~ /[\N{U+0041}-\N{U+005A}\N{U+0061}-\N{U+007A}]/) {
                $key_regex = $key_regex . '[' . $nth . '|' . to_fullwidth($nth) . ']';
            }
            #if full-width english letter
            elsif ($nth =~ /[\N{U+FF21}-\N{U+FF3A}\N{U+FF41}-\N{U+FF5A}]/) {
                $key_regex = $key_regex . '[' . $nth . '|' . to_halfwidth($nth) . ']';
            }
        }
        else {
            $key_regex = $key_regex . $nth;
        }
    }
    return $key_regex;
}
