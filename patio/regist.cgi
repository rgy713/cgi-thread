...
sub regist {
	...
	# アップロードファイル存在判定 TODO END
	my $imagefile_count = ($in{'increase_num'} ? 3 + $upl_increase_num : 3);
	my $upfile_exists = $image_upl && grep { $in{'upfile' . $_} } (1 .. $imagefile_count);
	my $error_img_idx = '';
	for ($i = 1; $i <= $imagefile_count; $i++) {
		if($fname{$i} ne ''){
			if($ctype{$i} =~ m/image\// || $ctype{$i} eq 'application/octet-stream') {
				my $img_type = &check_imagetype($in{'upfile'. $i});
				if($img_type eq ''){
					$error_img_idx = ($error_img_idx eq '') ? $i: $error_img_idx .'、' .$i;
				}
			} else {
				$error_img_idx = ($error_img_idx eq '') ? $i: $error_img_idx .'、' .$i;
			}
	...
	if($error_img_idx ne ''){
		&error("画像".$error_img_idx."枚目は未対応ファイルです。");
	}
	# ホストなどによる画像アップロードの無効
	...
	#TODO
	#if ($in_mail == 1 && $in{'email'} ne "") { &error("ホストは入力禁止です"); }
	if ($in_mail == 2 && $in{'email'} ne "") { &error("ホストは入力禁止です"); }
	if ($in_mail == 3 && $in{'email'} ne "") { &error("ホストは入力禁止です"); }
	...
	my $ID = $in{'ID'};
	my $historyID = $in{'historyID'};

	#投稿時の名前・スレッドタイトルの文字数チェック
	check_word_size();
...
sub min ($$) { $_[$_[0] > $_[1]] }
sub check_word_size{
	if(length($enc_cp932->decode($in{'sub'}))-length($enc_cp932->decode($in{'add_sub'})) > $sub_length){
		&error(" スレッドタイトルの文字数をオーバーしています。");
	}
	if($i_nam =~ /#/){
		if($i_nam =~ /◆/) {
			if (min(length($enc_cp932->decode((split(/◆/, $i_nam, 2))[0])),length($enc_cp932->decode((split(/#/, $i_nam, 2))[0])))>$name_length_trip){
				&error("名前の文字数をオーバーしています。");
			}
		}
		else{
			if (length($enc_cp932->decode((split(/#/, $i_nam, 2))[0]))>$name_length_trip){
				&error("名前の文字数をオーバーしています。");
			}
		}
	}else{
		if($i_nam =~ /◆/){
			...
			if (length($enc_cp932->decode($i_nam))>$name_length_no_trip){
				&error("名前の文字数をオーバーしています。");
			}
		}
	}
